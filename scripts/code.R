...
## Fit models and data analysis with appropriate data with only repeats that have a career length of 5-15 years

ggplot(Seasons_sub, aes(x= Seasons_sub$Age , y= Seasons_sub$`FG%`)) +
  geom_point(size=1, shape=23) +
  geom_smooth(method=lm) +
  labs(xlab="Age", ylab="FG%")
ggplot(Seasons_sub, aes(x= Seasons_sub$Age , y= Seasons_sub$`FT%`)) +
  geom_point(size=1, shape=1) +
  geom_smooth(method=lm) + (xlab="Age", ylab="FT%")
ggplot(Seasons_sub, aes(x= Seasons_sub$Age , y= Seasons_sub$AST)) + geom_point(size=1, shape=2) + geom_smooth(method=lm) + (xlab="Age", ylab="AST")
ggplot(Seasons_sub, aes(x= Seasons_sub$Age , y= Seasons_sub$`TS%`)) + geom_point(size=1, shape=3) + geom_smooth(method=lm) + (xlab="Age", ylab="TS%")
ggplot(Seasons_sub, aes(x= Seasons_sub$Age , y= Seasons_sub$PTS)) + geom_point(size=1, shape=20) + geom_smooth(method=lm) + (xlab="Age", ylab="PTS")
ggplot(Seasons_sub, aes(x= Seasons_sub$Age , y= Seasons_sub$G)) + geom_point(size=1, shape=22) + geom_smooth(method=lm) + (xlab="Age", ylab="Games")
